...
//		}
	...
	public static void PTH(String outFile) { //HEURISTIC
		try {
			...
			System.out.println ("accept demand = "+ acceptDemands);
			...
			for (int i = 0; i <dLst.size();i++) {
				...
				do {					
					...
					if (vstar.getCost() == -1) {
						acceptDemands--;
						System.out.println ("111no service");
						failed = true;
						break;
					}
					if (vstar.getIndex() == _d.getFunctions().length-1) {
						System.out.println ("111accepted");
						failed = false;
						break;
					}
				...
				while (!candidates.isEmpty()) {
					...
					if (node_result == _dest-1) {
						failed = false;
						UC+= bwD * _d.getFunctions()[result.getIndex()].getReq() * nodeCost[result.getNodeId()];
						out.write("function " + result.getIndex() + " of demand " + _d.idS() + " is placed to" + (node_result +1) );
						out.newLine();
						g1.setCap(node_result +1, (int) result.getCapacity());
						int prevNode = result.getPreNode();
						
						//update capacity
						while (prevNode!= _src -1) {
							myTuple nn = getTuple(prevNode, labels);
							UC+= bwD * _d.getFunctions()[nn.getIndex()].getReq() * nodeCost[nn.getNodeId()];
							out.write("function " + nn.getIndex() + " of demand " + _d.idS() + " is placed to" + (prevNode +1) );
							out.newLine();
							g1.setCap(prevNode +1, (int) nn.getCapacity());
							prevNode = nn.getPreNode();
						}
						result = getTuple(_src-1, labels);
						UC+= bwD * _d.getFunctions()[result.getIndex()].getReq() * nodeCost[result.getNodeId()];
						
						g1.setCap(_src, (int) result.getCapacity());
					...
					if (path !=null && !path.isEmpty()) {
						...
						while (node_result!= _src -1) {
							myTuple nn = getTuple(prevNode, labels);
							int prevIndex= nn.getIndex();
							for (int idx = prevIndex+1; idx < currIndex; idx ++) {
							...
							node_result = nn.getNodeId();
							currIndex = nn.getIndex();							
							prevNode = nn.getPreNode();
						}
						result = getTuple(_src-1, labels);
						if (result.getCost() > 0) {
							UC+= bwD * _d.getFunctions()[result.getIndex()].getReq() * nodeCost[result.getNodeId()];
							out.write("function " + result.getIndex() + " of demand " + _d.idS() + " is placed to"
									+ (_src));
							out.newLine();
							g1.setCap(_src, (int) result.getCapacity());
						}
				...
				if (failed) {
					acceptDemands--;	
					System.out.println ("222no service");
				} 
